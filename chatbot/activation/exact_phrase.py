...
class ExactPhrasePolicy(ActivationPolicy):
    """
    Activa el asistente únicamente cuando el mensaje completo coincide
    con una de las frases configuradas.

    La comparación ignora espacios exteriores y diferencias entre
    ...
    def __init__(
        ...
        if not prompt_message.strip():
            raise ValueError("prompt_message cannot be empty.")

        if prompt_cooldown is not None and prompt_cooldown.total_seconds() < 0:
            raise ValueError("prompt_cooldown cannot be negative.")

        self._phrases = frozenset(normalized_phrases)
    ...
    def _should_send_prompt(
        ...
        now = datetime.now(tz=state.prompt_sent_at.tzinfo)

        return now - state.prompt_sent_at >= self._prompt_cooldown
